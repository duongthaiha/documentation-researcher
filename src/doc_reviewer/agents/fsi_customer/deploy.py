"""Deploy the FSI customer agent to Azure AI Foundry as a prompt agent."""
...
def deploy(settings: Settings, *, dry_run: bool = True):
    """Create/version the FSI customer prompt agent in Foundry."""
    return deploy_prompt_agent(
        settings,
        FOUNDRY_AGENT_NAME,
        build_prompt_agent_definition(settings),
        description=DESCRIPTION,
        metadata={"persona": "fsi", "kind": "customer"},
        dry_run=dry_run,
    )
