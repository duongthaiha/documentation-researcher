"""Deploy the Manufacturing customer agent to Foundry as a prompt agent."""
...
def deploy(settings: Settings, *, dry_run: bool = True):
    """Create/version the Manufacturing customer prompt agent in Foundry."""
    return deploy_prompt_agent(
        settings,
        FOUNDRY_AGENT_NAME,
        build_prompt_agent_definition(settings),
        description=DESCRIPTION,
        metadata={"persona": "manufacturing", "kind": "customer"},
        dry_run=dry_run,
    )
